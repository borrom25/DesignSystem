...
| `modalContainer` filter popover | `Property 1=Variant2/3/4/5` | `columns[].filter`, `filters={tableController.filters}` | `dataTableFilter.list/columnFilter/numberRange/dateRange` + `useDataTableController` | - | Header filter icon opens the proper filter content through `DataTableFilterRenderer` |
...
| Column filter popovers | Yes | `columns[].filter` + `filters`; renderer chooses list/text/date/number range content by `filter.kind` |
...
- `modalContainer` filter variants are table internals. `DataTableHeader` renders the filter icon, wraps it in `Popover`, and `DataTableFilterRenderer` maps `filter.kind` to `DataTableListFilter`, `DataTableColumnFilter`, `DataTableDateFilter`, `DataTableDateRangeFilter` or `DataTableNumberRangeFilter`.
- Filter UI is functional only when the table receives both column filter config and filter state: `columns[].filter` plus `filters={tableController.filters}` from `useDataTableController({ columns })` or an equivalent `DataTableFiltersState`.
...
| `modalContainer` filter variants | `columns[].filter` + `filters` | Runtime owns the popover and filter renderer | Keep filter setup on columns; do not mount filter modal containers directly |
