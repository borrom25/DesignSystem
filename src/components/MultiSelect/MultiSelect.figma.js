// url=https://www.figma.com/design/gPjMJwL1jc8V4G6x4lZJDa/0.-%D0%94%D0%B0-%D0%BF%D0%BE%D0%BC%D0%BE%D0%B6%D0%B5%D1%82-%D0%BC%D0%BD%D0%B5-%D0%A2%D0%B5%D0%BF%D0%BB%D0%BE%D0%B2?node-id=5351-1298
// source=src/components/MultiSelect/MultiSelect.tsx
...
const isFilled =
  filledState === "on" || state === "filled" || state === "filledHover" || state === "disable";
const defaultOpen = state === "selected";
const clearable = state === "filledHover";

// Temporary mapping: Figma component doesn't expose selected items as structured list.
const options = figma.tsx`[
  { value: "selected", label: "${filledText || "Selected item"}" },
...
const hintErrorProp = showHint && isError ? figma.tsx`hintError="${hintError}"` : "";
const placeholderProp = showPlaceholder
  ? figma.tsx`placeholder="${placeholder}"`
  : figma.tsx`placeholder="Select items..."`;
...
export default {
  example: figma.tsx`
    <MultiSelect
      ...
      ${disabled ? "disabled" : ""}
      ${isError ? "error" : ""}
      ${isFilled ? figma.tsx`defaultValue={${selectedValues}}` : ""}
    ...
    />
  `,
  imports: ['import { MultiSelect } from "borrom-ds-test"'],
  id: "multi-select",
  metadata: {
    nestable: true,
    props: {
