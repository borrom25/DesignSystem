...
$storybookCmd = Join-Path $projectRoot "node_modules\\.bin\\storybook.cmd"
...
if (Test-Path $storybookCmd) {
  $filePath = $storybookCmd
  $args = @("dev", "--host", "127.0.0.1", "--port", "$selectedPort", "--exact-port")
} else {
  $filePath = "npm.cmd"
  $args = @("run", "dev", "--", "--host", "127.0.0.1", "--port", "$selectedPort")
}
